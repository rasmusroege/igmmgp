classdef Absgmmgp_unconstrained<handle
    methods
        function obj=Absgmmgp_unconstrained()
        end
...
%                     w2_new_vector=(1./(1+1./mean(sub(l).w2)-1).*exp(-stepsize_w2*randn))*ones(size(sub(l).w2));
                    w2_new_vector=exp(log(mean(sub(l).w2))+stepsize_w2*randn)*ones(size(sub(l).w2));
        ...
        function sample_sig2homo_w2het(obj,X,maxiter)
            ...
            for j=1:maxiter
                ...
                for l=1:obj.par.nsubjects
                    ...
                    %                     logPdiff=-0.5*(1/sig2_new-1/sub(l).sig2(i))*sum(X{l}(:,i).^2)-T*0.5*(log(sig2_new)-log(sub(l).sig2(i)));
                    lp_new=-0.5*T*obj.par.N*log(2*pi)-0.5*T*sum(log(sig2_new_vector))-K/2*sum(log(s(l).D))-0.5*norm(bsxfun(@times,X{l},1./sqrt(sig2_new_vector')),'fro')^2;
...
%                     w2_new_vector=max(1./(1+(1./sub(l).w2-1).*exp(-stepsize_w2*randn(obj.par.N,1))),1e-6*ones(obj.par.N,1));
                    w2_new_vector=max(exp(log(sub(l).w2)+stepsize_w2*randn(obj.par.N,1)),1e-6*ones(obj.par.N,1));
                    for i=1:obj.par.N
                        k=obj.par.z(i);
        ...
        function sample_sig2homo_w2homo(obj,X,maxiter)
            ...
            for j=1:maxiter
                for l=1:obj.par.nsubjects
                    ...
                    %                     logPdiff=-0.5*(1/sig2_new-1/sub(l).sig2(i))*sum(X{l}(:,i).^2)-T*0.5*(log(sig2_new)-log(sub(l).sig2(i)));
                    lp=-0.5*T*obj.par.N*log(2*pi)-0.5*T*sum(log(sig2_new_vector))-K/2*sum(log(obj.ss(l).D))-0.5*norm(bsxfun(@times,X{l},1./sqrt(sig2_new_vector')),'fro')^2;
...
%                     w2_new_vector=1./(1+(1./mean(obj.par.sub(l).w2)-1).*exp(-stepsize_w2*randn))*ones(size(obj.par.sub(l).w2));
                    w2_new_vector=exp(log(mean(obj.par.sub(l).w2))+stepsize_w2*randn)*ones(size(obj.par.sub(l).w2));
                    nu_tmp=zeros(K,1);
                    for i=1:K
