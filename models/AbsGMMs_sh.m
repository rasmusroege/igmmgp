classdef(Abstract) AbsGMMs_sh < handle
    properties
        
    end
    
    methods
        function obj=AbsGMMs_sh()
        end
        ...
        function xmeans=getpostmeans(obj)
            ...
            for l=1:obj.par.nsubjects
                xmeans(:,:,l)=bsxfun(@rdivide,1/(1+obj.par.sub(l).lambda)*(bsxfun(@plus,obj.ss(l).x_avg,obj.par.sub(l).lambda*obj.par.sub(1).mu0)),obj.par.nk');                
            end
        end
        ...
        function [E_Sigma]=compExpSig(obj)
            ...
            for l=1:obj.par.nsubjects
                for k=1:length(obj.par.nk)
                    E_Sigma{l}(:,:,k)=diag((repmat(obj.ss(l).Sigma_avg/obj.par.T,obj.par.T,1)+2*obj.par.sub(l).gamma+obj.par.sub(l).lambda*obj.par.sub(l).mu0.^2-1/(obj.par.nk(k)+obj.par.sub(l).lambda)*(obj.ss(l).x_avg(:,k)+obj.par.sub(l).lambda*obj.par.sub(l).mu0).^2)./(obj.par.nk(k)+obj.par.sub(l).v0-1));
                end
            end
        end        
