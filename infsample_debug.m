function [z,llh,noc,cputime,strout,best_sample,amis]=infsample_debug(X,model,opts)
...
if isfield(opts,'best_sample');best_sample=opts.best_sample;else;best_sample=copy(model);end;
...
%             split_sample(X,model,debug);
    ...
    if isempty(zt)
        ...
        strout{iter}=sprintf(' %12.0f | %12.4e | %12.4e | %12.0f |%s %7.3f | %s\n',iter,llh(end), dllh, getnoc(model),modelstr(1:5),amis(end),datestr(now));
...
for n=sample_idx
    ...
    if debug
        % verify that all sufficient statistics are correct
        mf=fields(model.ss);
        m=copy(model);
        m.calcss(X);
        errorlist={};
        for j=1:length(mf)
            for l=1:model.par.nsubjects
                tmpa=m.ss(l).(mf{j});
                tmpb=model.ss(l).(mf{j});
                if norm(tmpa(:)-tmpb(:))/norm(tmpa(:))>1e-6
                    errorlist{end+1}=mf{j};
                end
            end
        end
